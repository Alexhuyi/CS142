    ...
    User.findOne({_id: id},(err, user) => {
        if (err) {
            ...
            Photo.find({user_id: id}, (err1,photos) => {
                ...
                if(photos.length ===0){ 
                  console.log("No photo of this user",newUser);
                  response.status(200).send(newUser);
                  return;
                }
                let recent = photos.sort((photo1,photo2)=> new Date(photo2.date_time) - new Date(photo1.date_time))[0];
                newUser.recentPhoto = {owner_id:recent.user_id,date_time:recent.date_time,file_name:recent.file_name};
                let most = photos.sort((photo1,photo2) => photo2.comments.length - photo1.comments.length)[0];
                newUser.mostCommentsPhoto = {owner_id:most.user_id,commentLength:most.comments.length,file_name:most.file_name};
                //console.log(newUser);
                response.status(200).send(newUser);
            }); 
        }
...
app.post("/commentsOfPhoto/:photo_id",function(request, response) {
    ...
    Photo.findOne({_id:photo_id}, function(err, photo) {
        ...
        let record = new Record({
            object_id: newcomment._id,
            parent_object_id: photo._id,
            user_id: request.session.user_id,
            type: "Comment"
          });
        record.save();
...
//     Photo.find({user_id:request.session.user_id}, (err,photos) => {
//         if(err){
...
//         let newphotos = photos.sort((photo1,photo2)=> new Date(photo1.date_time) < new Date(photo2.date_time));
//         let recent = newphotos[0];
        
//         let recentPhoto = {owner_id:recent.user_id,date_time:recent.date_time,file_name:recent.file_name};
//         //let most = photos.sort((photo1,photo2) => photo1.comments.length < photo2.comments.length)[0];
//         //info.mostCommentsPhoto = {owner_id:most.user_id,commentLength:most.comments.length,file_name:most.file_name};
//         console.log(recentPhoto);
//         response.status(200).send(recentPhoto);
//     }); 
// });
...
app.get("/activity", (request, response) => {
    ...
    Record.find({}).lean().sort({date_time:-1}).limit(5)
      .exec((err, records) => {
        if (err) {
        ...
        async.forEachOf(
          ...
          (element, index,callback) => {
            switch (element.type) {
              ...
              case "Comment": {
                ...
                Photo.findById(element.parent_object_id)
                  ...
                  .exec((err5, photo) => {
                    ...
                    item.comment = photo.comments
                    .filter((elementSub) => elementSub._id === element._id);
  
                    delete photo.comments;
                    item.type = "Comment";
                    item.photo = photo;
                    item.date_time = element.date_time;
                    info[index]=item;
                    callback();
                  });
                break;
              ...
              case "Login": {
                User.findOne({_id:element.user_id}).lean().exec((err7, user) => {
                    if (err7) {
                      console.error(err7);
                      return;
                    }
              ...
              default: {
                console.log("Activity type not matched");
                callback();
              }
            }
          },
          (error) => {
            if (error) {
              console.log("error in getting activity");
              response.status(400).send("Error in Activity.");
              return;
            }
            console.log("activity data",info);
            response.status(200).send(info);
          }
        );
      });
